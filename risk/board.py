...
xs += ['to_territory_id','to_plater_id', 'to_armies']
...
class Board(object):
    ...
    def create(cls, n_players):
        ...
        Returns:
            Board: A board with territories randomly
    ...
    def is_valid_attack_path(self, path):
        ...
        a player's armies cannot move through
        territories they already occupy;
        they must move through enemy territories.
        All valid attacks, therefore, will follow a
        path of starting on one player's territory and
        moving trough enemy territories.
    ...
    def cost_of_attack_path(self, path):
        '''
        The cost of an attack path is the total
        number of enemy armies in the path.
        In other words, it is the total number of
        armies in the subpath starting at the second vertex.

        Args:
            path ([int]): a list of territory_ids
            which must be a valid attack path
    ...
    def cheapest_attack_path(self, source, target):
        ...
        cheapest valid attack path between two territories if such
        a path exists.
        There may be multiple valid cheapest attack paths (in which
        case it doesn't matter which this function returns),
        ...
        Returns:
            ...
            valid attack path; if no path exists,
    ...
    def continent(self, continent_id):
        """
        Create a generator of all territories that
        belong to a given continent.
    ...
    def num_foreign_continent_territories(self, cid, pid):
        """
        ...
        """
        return sum(1 if t.player_id != pid else 0 for t in self.continent(cid))
    ...
    def fortify(self, from_territory, to_territory, n_armies):
        ...
        Args:
            from_territory (int): Territory_id of the
            territory where armies leave.
            to_territory (int): Territory_id of the
            territory where armies arrive.
